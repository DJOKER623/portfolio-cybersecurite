...
{
    stackSlotSize = 256;
    stackNumSlots = UserStacksAreaSize / stackSlotSize;
    stackBitmap = new BitMap(stackNumSlots);
    ASSERT(stackBitmap != NULL);
    stackBitmap->Mark(0);

    threadCountLock = new Lock("threadCountLock");
    ASSERT(threadCountLock != NULL);
    threadCount = 1;
...
int AddrSpace::AllocateUserStack()
{
    ...
    int slot = stackBitmap->Find(); // trouve un slot libre
    if (slot == -1)
    {
        DEBUG('t', "AddrSpace::AllocateUserStack: plus de slots disponibles !\n");
        threadCountLock->Release();
        ASSERT(false); // plus de piles disponibles
    }
    threadCountLock->Release();
    int top = numPages * PageSize - slot * stackSlotSize; // calcul du sommet
    DEBUG('a', "AllocateUserStack: slot=%d, top=0x%x\n", slot, top);
    return top;
#else
    return 0; // hors USER_PROGRAM
#endif
}
...
void AddrSpace::FreeUserStack(int top)
{
#ifdef USER_PROGRAM
    int slot = (numPages * PageSize - top) / stackSlotSize;

    if (slot < 0 || slot >= stackNumSlots)
    {
        DEBUG('a', "FreeUserStack: invalid top=0x%x (slot=%d)\n", top, slot);
        return; // éviter le crash, mais signaler
    }

    stackBitmap->Clear(slot);
    DEBUG('a', "FreeUserStack: libération du slot %d (top=0x%x)\n", slot, top);
#endif
}
